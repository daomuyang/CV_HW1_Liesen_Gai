...
def load_fashion_mnist(val_size=10000):
    ...
    # 这里使用Tensorflow只是为了加载数据，后续的数据处理和训练是手动完成的
...
def plot_history(history):
    ...
    plt.xlabel('Training Epoch', fontsize=12)  # 补全x轴标签
    plt.ylabel('Cross-Entropy Loss Value', fontsize=12)  # 补全y轴标签
    ...
    plt.xlabel('Training Epoch', fontsize=12)  # 补全x轴标签
    plt.ylabel('Accuracy (0-1)', fontsize=12)  # 补全y轴标签
    plt.legend(fontsize=12)
    plt.grid(alpha=0.3)
...
def visualize_weights(model):
    W1 = model.params['W1']
    hidden_dim = W1.shape[1]  # 获取隐藏层全部神经元数量
    
    # 自动计算子图的行列数（接近正方形，适配任意隐藏层大小）
...
if __name__ == '__main__':
    ...
    # 或者直接用固定最优参数
    # best_lr, best_hidden_dim, best_l2 = 0.05, 256, 0.001
